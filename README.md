...
> **Not 1:** Site dosyaları depo kökündedir — Vercel/Netlify/GitHub Pages ayar gerektirmez.
>
...
Bu depo **web sürümünü** barındırır (Vercel/Netlify kökten yayınlar). Mobil ve
masaüstü sürümler yerelde geliştirilir:
...
mobil/              📱 Android sürümü (repoya girmez, yerelde geliştirilir)
PC & Desktop/       🖥️ Windows sürümü → github.com/AlperenUnalHacking/NesilAI-PC
```

### Android APK derleme (yerel)

```bash
cd mobil
npm install
npm run android   # kök site dosyalarını dist/'e kopyalar, senkronize eder, APK derler
```

APK çıktısı: `mobil/android/app/build/outputs/apk/debug/app-debug.apk`

### Masaüstü (Windows) derleme (yerel)
<arg_value><b88a6f17>```bash
cd "PC & Desktop"
npm install
npm start        # pencerede çalıştırır (test için)
npm run dist     # installer + portable exe üretir
```
